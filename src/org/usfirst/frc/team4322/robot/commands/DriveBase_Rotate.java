...
//    	Robot.driveBase.leftMaster.set(ControlMode.MotionMagic, -ticks);
	...
	{
		Robot.driveBase.leftMaster.set(ControlMode.PercentOutput, Math.copySign(.3, degrees));
		Robot.driveBase.rightMaster.set(ControlMode.PercentOutput, -Math.copySign(.3, degrees));
	}
	@Override 
    protected void end()
    {
	...
	{
		if (Math.abs(Robot.driveBase.getAngle()) >= Math.abs(degrees))
		{
			return true;
		}
		else
